...
class PlayersWTPipeline(AbstractWTPipeline, ABC):
    ...
    def process_item(self, item, spider):
        ...
        self.webhook_url = (
            WEBHOOK_PLAYERS if self.table == "players_instant" else WEBHOOK_DAY
        )
    ...
    def check_leavers(self):
        ...
        leavers = [
            member for member in database_members if member[0] not in self.members
        ]
        if len(leavers) < 10:
...
class ClansWTPipeline(AbstractWTPipeline, ABC):
    ...
    def process_item(self, item, spider):
        super().process_item(item, spider)
        self.webhook_url = (
            WEBHOOK_SQUADRONS if self.table == "squadrons_instant" else WEBHOOK_DAY
        )
        self.update_data(item)
        return item
    ...
    def build_embed(self):
        ...
        self.first_message.set_title(
            "Leaderboard"
            if self.table == "squadrons_instant"
            else "Results for the day"
        )
        for message in {self.first_message, self.second_message}:
