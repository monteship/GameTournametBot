...
def is_abandon(personal: list):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        abandoners = set(sql_personal) - set(personal)
        if abandoners:
            for abandoner in abandoners:
                print(f"Imposter: {abandoner}")
                try:
                    sql.execute(f"SELECT name, points FROM players WHERE name = '{abandoner}'")
                    imposter_data = sql.fetchall()
                    delete_player(sql, abandoner)
                    conn.commit()
...
def abandoned_notify(imposter_name, imposter_points):
    ...
    webhook.add_embed(DiscordEmbed(
        title=f"Нас покинув {imposter_name} з очками в кількості: {imposter_points}",
        color="000000",
