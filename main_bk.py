...
class ScheduleSpiders:
    ...
    def schedule_daily_leader_board_parsing(self):
        self.schedule.every().day.at("22:15").do(
            lambda: os.system(
                f"{PY_PATH} scrapy crawl LeaderBoardStats -a table_name=squadrons_daily"
            )
        )
        ...
        for parsing_time in clans_parsing_time:
            self.schedule.every().day.at(parsing_time).do(
                lambda: os.system(
                    f"{PY_PATH} scrapy crawl LeaderBoardStats -a table_name=squadrons_instant"
                )
            )
    ...
    def schedule_daily_players_parsing(self):
        self.schedule.every().day.at("22:16").do(
            lambda: os.system(
                f"{PY_PATH} scrapy crawl PlayerStats -a table_name=players_daily"
            )
        )

        self.schedule.every().minute.do(
            lambda: os.system(
                f"{PY_PATH} scrapy crawl PlayerStats -a table_name=players_instant"
            )
        )
